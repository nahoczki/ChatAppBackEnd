...
router.get('/', verify, async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            chatrooms: requestingUser.chatrooms
        });
    } catch (e) {
...
router.post('/', verify, async (req, res) => {
    ...
    try {
        ...
        await requestingUser.updateOne({ $addToSet: { chatrooms: [savedChatRoom] },});
        await recipient.updateOne({ $addToSet: { chatrooms: [savedChatRoom] },});
